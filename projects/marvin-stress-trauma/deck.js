...
// 6 Methods: sample
s = base("Survey of 1,766 adults, January to March 2024. Six states: five conflict-affected — Benue, Borno, Enugu, Rivers, Sokoto — and Ogun as a non-conflict comparison, which anchors the low end of the trauma gradient. Stratified multi-stage cluster design: state strata, 572 community clusters, community and IDP-camp settings. Analyses account for the design throughout.");
title(s, "Sample");
body(s, [
 { text:"1,766 adults, January–March 2024" },
...
// 9 Results: IRR table
s = base("Read one row aloud. Among adults with no trauma exposure, each additional stressor raises PTSD prevalence by 73 percent. Among adults with five or more event types, nothing — a null. Depression and anxiety do the same. The interaction is significant for all three outcomes, p at or below 0.0006. The red row is the finding: stress sensitivity is concentrated where trauma exposure is lowest.");
title(s, "Per-stressor prevalence ratio, by trauma exposure");
{
 const rows = [
  [{text:"Trauma"},{text:"PTSD"},{text:"Depression"},{text:"Anxiety"}],
  [{text:"0 events"},{text:"1.73 (1.37–2.18)"},{text:"1.53 (1.18–1.98)"},{text:"1.52 (1.22–1.89)"}],
  ...
  [{text:"Interaction p"},{text:"<0.0001"},{text:"0.0006"},{text:"0.0001"}],
...
// 11 Absolute scale
s = base("Why this matters for programming: the absolute scale. Going from zero to four stressors adds 22 percentage points of PTSD prevalence among adults with no trauma exposure — from 7 to 29 percent. The same contrast among adults with five event types adds nothing, minus three points with a confidence interval straddling zero, against a baseline near 40 percent. Depression: plus 22 versus plus 3. Anxiety: plus 17 versus minus 4. All three nulls at high exposure.");
title(s, "Four additional stressors, in percentage points");
s.addText([{ text:"+21.9", options:{ color:RED }}], { x:1.2, y:1.7, w:3.4, h:1.2, fontFace:F, fontSize:60, isTextBox:true, margin:0, align:"center" });
s.addText("no trauma exposure\n(95% CI 7.7–36.1)", { x:1.2, y:2.9, w:3.4, h:0.8, fontFace:F, fontSize:14, color:BLK, isTextBox:true, margin:0, align:"center" });
s.addText("−3.0", { x:5.4, y:1.7, w:3.4, h:1.2, fontFace:F, fontSize:60, color:BLK, isTextBox:true, margin:0, align:"center" });
s.addText("five or more events\n(95% CI −19.5 to +13.6)", { x:5.4, y:2.9, w:3.4, h:0.8, fontFace:F, fontSize:14, color:BLK, isTextBox:true, margin:0, align:"center" });
s.addText("Depression: +21.9 vs +2.8   ·   Anxiety: +16.6 vs −4.4", { x:0.6, y:4.2, w:8.8, h:0.5, fontFace:F, fontSize:16, color:BLK, isTextBox:true, margin:0, align:"center" });

// 12 Robustness
s = base("The result is not an artifact of any cutoff. PCL-5 at 33, PHQ-9 at 15, GAD-7 at 7, and the continuous symptom scores all show the interaction at p of 0.0001 or below. The categorical specification from earlier versions of this work agrees.");
title(s, "Robustness");
body(s, [
 { text:"Alternative thresholds (PCL-5≥33, PHQ-9≥15, GAD-7≥7): all p≤0.0001" },
