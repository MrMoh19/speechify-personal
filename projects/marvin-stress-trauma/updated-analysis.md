...
- Five events (corrected basis): PTSD −7.3 pp (p=0.52; CI pending design check); depression +3.5 pp (−7.7 to +14.6, p=0.54); anxiety −4.0 pp (−15.1 to +7.1, p=0.48). Null in all three.

**Predicted prevalence (adjusted, corrected spline basis):** at zero events, PTSD rises from 6.6% with no stressors to 28.3% at four; at five events it declines from 39.6% with no stressors to 32.2% at four (29.1% at six) — high, with a modest decline. RD at five events: −7.3 pp (null, p=0.52). CAUTION: the corrected-margins run showed 70 PSUs / N=1,712 instead of 558 / 1,706 — DESIGN DECISION (24 Sep, final): PSU = community (cluster_id), as in all original runs and Salma's paper; methods text stays low-key about clustering level. The ward-clustered margins run (70 PSUs) gave near-identical points; definitive numbers come from the full do-file rerun under cluster_id. Depression and anxiety corrected margins still to run (earlier t=5/t=7 predictions used a wrong spline-basis constant; t=0–3 rows were correct). Depression: 5.4%→29.4% at zero events; 35.3%→38.7% at five. Anxiety: 7.1%→24.9% at zero events; 40.2%→36.2% at five.
